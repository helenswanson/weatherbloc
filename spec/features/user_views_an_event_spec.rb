...
# - TODO If an event is over, it cannot be joined.
  ...
  context 'authenticated user'
    ...
    context 'authenticated non-host'
      before :each do
        @user = FactoryGirl.create(:user)
        login(@user)
        @event = FactoryGirl.create(:event)
      ...
      scenario 'attendee views page for event with >= 3 attendees'
      ...
      context 'users not associated with a >= 3 attendees event' do
        ...
        scenario 'attendeee views page for event'
          expect(page).to_not have_button('RSVP')
          expect(page).to have_button('Withdraw RSVP')
        end

        scenario 'user without RSVP views event page'
